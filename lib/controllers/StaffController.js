...
exports.findByApplication = function (appID, callback) {
    if (typeof appID == mongoose.Schema.ObjectId) {
        Staff.find({ ApplID: appID }, null, function (err, docs) {
            callback(err, docs);
        });
    }
};
...
exports.removeByApplication = function (appID, callback) {
    if (typeof appID == mongoose.Schema.ObjectId) {
        Staff.remove({ ApplID: appID }, function (err) {
            callback(err);
        });
    }
};
...
//
// The function will attempt to find the fallback staffer for an application
exports.findFallbackStaffer = function (appID, callback) {
    if (typeof appID == mongoose.Schema.ObjectId) {
        Staff.findOne({ ApplID: appID, IsFallback: true }, null, function (err, doc) {
            callback(err, doc);
        });
    }
};