...
exports.currentSegment = function (_id, callback) {
    var today = new moment().utc().hour(0).toDate();
    var foundSegment = null;
    this.findById(_id, function(err, doc){
        if(doc){
            doc.Segments.forEach(function (segment) {
                var currStartDate = new moment(segment.StartDate).utc().hour(0);
                var currEndDate = new moment(segment.EndDate).utc().hour(0);
...
exports.findSecondary = function (_id, callback) {
    this.currentSegment(_id, function (err, doc) {
        if (doc){
