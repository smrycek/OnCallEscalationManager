...
exports.handleIncoming = function (req, res) {
    ...
    applicationController.findByPhone(convertedPhone, function (err, application) {
        if (!err) {
            var retTwiml = "";
            conference.exists(application.Name, function (err, doesExist) {
                if (!doesExist && !err) {
...
exports.handleConference = function (req, res) {
    ...
    handleTwilioError(res, "Incorrect key presses received. Received: " + keyPresses == null ? keyPresses : "None");
}

function handleTwilioError(res, message) {
    console.log("----------------HANDLE TWILIO ERROR-------------/n" + message);
    res.writeHead(200, { 'Content-Type': 'text/xml' });
