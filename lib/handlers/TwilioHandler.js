...
exports.handleIncoming = function (req, res, next) {
    ...
    .then(function (doesExist) {
        if (!doesExist) {
            ...
            var pressKeyText = 'You have dialed the on call escalation number for ' + foundApplication.Name + '. Press 1 to contact the currently on-call staff member.';
...
exports.handleConference = function (req, res, next) {
    ...
    if ((keyPresses && keyPresses.toString() === '1') || (!keyPresses && answeredBy === 'human')) {
        res.writeHead(200, { 'Content-Type': 'text/xml' });
        res.write(twimlGenerator.Start());
        if (!keyPresses && answeredBy === 'human') {
            res.write(twimlGenerator.Say("You have been called as part of an escalation for " + confParams.Name + ". You will now be connected to the call."));
        }
...
function handleTwilioError(res, message) {
    ...
    res.writeHead(200, { 'Content-Type': 'text/xml' });
    res.end(twimlGenerator.Error(message));
}
